...
export default function BotSettings({ token, guildId }: Props) {
  ...
  // Load text channels + current setting whenever the guild changes
    ...
    <div className="card p-5 space-y-4">
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-xs text-app-muted">
          ...
          <span className="ml-1.5 text-app-border font-normal">(where "added via web" messages go)</span>
        </label>
        ...
        <p className="text-xs text-app-border leading-relaxed">
          When set to <strong className="text-app-muted">Auto-detect</strong>, the bot looks for a
          channel named <strong className="text-app-muted">#musicbot</strong> first, then falls back
          to the server's system channel.
        </p>
      </div>
    </div>
  )
